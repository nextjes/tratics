...
import { MilliSecond, TaskStatus } from "~/engine/term";
...
describe("Task.ready", () => {
  it("should initialize a task with ready status and zero elapsed time", () => {
    const task = Task.ready(new MilliSecond(3000));
...
describe("Task.start", () => {
  it("should change the status to running when it is ready", () => {
    const task = Task.ready(new MilliSecond(3000)).start();

    expect(task.status()).toEqual(TaskStatus.Running);
  });

  it.concurrent.each([
    [Task.ready(new MilliSecond(3000)).start(), TaskStatus.Running],
    [
      Task.ready(new MilliSecond(3000)).start().terminate(),
...
describe("Task.terminate", () => {
  it("should change the status to terminated when it is running", () => {
    const task = Task.ready(new MilliSecond(3000)).start().terminate();

    expect(task.status()).toEqual(TaskStatus.Terminated);
  });

  it.concurrent.each([
    [Task.ready(new MilliSecond(3000)), TaskStatus.Ready],
    [
      Task.ready(new MilliSecond(3000)).start().terminate(),
      TaskStatus.Terminated,
    ],
